<?php
	...
	class QualityMetaData extends SQLite3{
		function __construct(){
			$this->open('quality_database.db');		
		}
		
		/*
		 * Get Remote Table Count From External Server
		 */
		 public function getQualityTableCount(){
		 	$table_type = "BASE TABLE";
			$table_name1 = "^CWQT";
			$table_name2 = "^CWQT.*CORE";
			$table_name3 = "^CWQT.*TEST";
			$table_name4 = "^CWQT.*TEST1";
		 ...
		 public function updateMetaData(){
			...
			if(pg_num_rows($query_result) > 0){
				while($row = pg_fetch_array($query_result)){
					$count = $row["mcount"];
					$this->insertMetaRecord('quality', $count);
				}
			}
		 }
		 
		 #Get All Installation Tables
		 public function getQualityTables(){
		 	$table_type = "BASE TABLE";
			$table_name1 = "^CWQT";
			$table_name2 = "^CWQT.*CORE";
			$table_name3 = "^CWQT.*TEST";
			...
			$this->exec('DELETE FROM _QUALITY_TABLE');
			$query_result = $obj->runQuery($sql,'odk_prod');
			if(pg_num_rows($query_result) > 0){
				while($row = pg_fetch_array($query_result)){
					$tablename = $row["table_name"];
					$this->insertQualityRecord($tablename);
				}
				return TRUE;
			}	
			else{
		 ...
		 public function getTableCountStatus(){
		 	$remotecount = $this->getQualityTableCount();
		 ...
		 #Insert Installation
		 public function insertQualityRecord($tablename){
		 	try{
				$this->exec("INSERT INTO _QUALITY_TABLE (TABLE_NAME) VALUES('$tablename')");
			}
			catch(Exception $e){
		 ...
		 public function getQualityRecords(){
		 	$tables;
		 	try{
				$result = $this->query("SELECT TABLE_NAME FROM _QUALITY_TABLE ORDER BY TABLE_NAME");
		 ...
		 public function createTables(){
		 	try{
				...
				$this->exec('CREATE TABLE _QUALITY_TABLE(ID INTEGER PRIMARY KEY AUTOINCREMENT, TABLE_NAME TEXT NOT NULL)');
			}
			catch(Exception $e){
				echo $this->lastErrorMsg();
			}
