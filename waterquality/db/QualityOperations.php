<?php
	include 'QualityMetaData.php';
	include 'QualitySurveyFields.php';
	...
	class QualityOperations {
		...
		public function generateSQL(){
			$obj = new QualityMetaData();
			$obj -> main();	
			$fields = new QualitySurveyFields();
			$qualities = $obj -> getQualityRecords();	
			$ext = new Funcs();
			$dbname = 'odk_prod';
		...
		public function generateRows($result, $fields){			
			while ($row = pg_fetch_array($result, NULL, PGSQL_ASSOC)){
				$sql = "INSERT INTO waterqualitysurvey.waterquality_survey($fields) VALUES(";
				$count = count($row);
				$y = 0;
				while ($y < $count){
					$c_row = current($row);					
