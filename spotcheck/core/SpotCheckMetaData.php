<?php
	...
	class SpotCheckMetaData extends SQLite3{
		 ...
		 */
		 public function getSpotCheckTableCount(){
		 	$table_type = "BASE TABLE";
			$table_name1 = "^KS003";
			$table_name2 = "^KS003.*CORE";
			$table_name3 = "^KS003.*TEST";
			$table_name4 = "^KS003.*TEST1";
		 ...
		 public function updateMetaData(){
			...
			if(pg_num_rows($query_result) > 0){
				while($row = pg_fetch_array($query_result)){
					$count = $row["mcount"];
					$this->insertMetaRecord('spotcheck', $count);
				}
			}
		 }
		 
		 #Get All Installation Tables
		 public function getSpotCheckTables(){
		 	$table_type = "BASE TABLE";
			$table_name1 = "^KS003";
			$table_name2 = "^KS003.*CORE";
			$table_name3 = "^KS003.*TEST";
			...
			$this->exec('DELETE FROM _SPOTCHECK_TABLE');
			$query_result = $obj->runQuery($sql,'odk_prod');
			if(pg_num_rows($query_result) > 0){
				while($row = pg_fetch_array($query_result)){
					$tablename = $row["table_name"];
					$this->insertSpotCheckRecord($tablename);
				}
				return TRUE;
			}	
			else{
		 ...
		 public function getTableCountStatus(){
		 	$remotecount = $this->getSpotCheckTableCount();
		 ...
		 #Insert Installation
		 public function insertSpotCheckRecord($tablename){
		 	try{
				$this->exec("INSERT INTO _SPOTCHECK_TABLE (TABLE_NAME) VALUES('$tablename')");
			}
			catch(Exception $e){
		 ...
		 public function getSpotCheckRecords(){
		 	$tables;
		 	try{
				$result = $this->query("SELECT TABLE_NAME FROM _SPOTCHECK_TABLE ORDER BY TABLE_NAME");
		 ...
		 public function createTables(){
		 	try{
				...
				$this->exec('CREATE TABLE _SPOTCHECK_TABLE(ID INTEGER PRIMARY KEY AUTOINCREMENT, TABLE_NAME TEXT NOT NULL)');
			}
			catch(Exception $e){
				echo $this->lastErrorMsg();
			}
