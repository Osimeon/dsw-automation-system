<?php
	include 'SpotCheckMetaData.php';
	include 'SpotCheckSurveyFields.php';
	
	class SpotCheckOperations {
		public function __construct(){
			$this->generateSQL();
		}
		
		public function generateSQL(){
			$obj = new SpotCheckMetaData();
			$obj -> main();	
			$fields = new SpotCheckSurveyFields();
			$spotchecks = $obj -> getSpotCheckRecords();	
			$ext = new Funcs();
			$dbname = 'odk_prod';
			
			foreach($spotchecks as $spotcheck){				 
				$currtable = $spotcheck['tablename'];
		...
		public function checkForRecord($_uri){
			$sql = "SELECT count(_uri) as recs FROM spotcheck_survey.spotcheck_survey WHERE _uri = '$_uri'";
			$ext = new Funcs();
		...
		public function deleteRecord($result){	
			...
			while ($row = pg_fetch_array($result, NULL, PGSQL_ASSOC)){
				...
				if($this -> checkForRecord($_uri) == true){
					$sql = "DELETE FROM spotcheck_survey.spotcheck_survey WHERE _uri = '$_uri'";
					$query_result = $ext -> runQuery($sql,'dsw_db_v2');
				}
			}
		}
		...
		public function generateRows($result, $fields){			
			while ($row = pg_fetch_array($result, NULL, PGSQL_ASSOC)){
				...
				if($this -> checkForRecord($_uri) != true){	
					...
					$sql = "INSERT INTO spotcheck_survey.spotcheck_survey($fields) VALUES(";
					$count = count($row);
					$y = 0;
					while ($y < $count){
						$c_row = current($row);					
