<?php
	include 'SSMMetaData.php';
	include 'SSMSurveyFields.php';
	...
	class SSMOperations {
		...
		public function generateSQL(){
			$obj = new SSMMetaData();
			$obj -> main();	
			$fields = new SSMCheckSurveyFields();
			$ssms = $obj -> getSSMRecords();	
			$ext = new Funcs();
			$dbname = 'odk_prod';
			
			foreach($ssms as $ssm){				 
				$currtable = $ssm['tablename'];
		...
		public function checkForRecord($_uri){
			$sql = "SELECT count(_uri) as recs FROM ssm_survey.ssm_survey WHERE _uri = '$_uri'";
			$ext = new Funcs();
		...
		public function deleteRecord($result){	
			...
			while ($row = pg_fetch_array($result, NULL, PGSQL_ASSOC)){
				...
				if($this -> checkForRecord($_uri) == true){
					$sql = "DELETE FROM ssm_survey.ssm_survey WHERE _uri = '$_uri'";
					$query_result = $ext -> runQuery($sql,'dsw_db_v2');
				}
			}
		}
		...
		public function generateRows($result, $fields){			
			while ($row = pg_fetch_array($result, NULL, PGSQL_ASSOC)){
				...
				if($this -> checkForRecord($_uri) != true){	
					...
					$sql = "INSERT INTO ssm_survey.ssm_survey($fields) VALUES(";
					$count = count($row);
					$y = 0;
					while ($y < $count){
						$c_row = current($row);					
