<?php
	...
	class VerificationMetaData extends SQLite3{
		function __construct(){
			$this->open('verification_database.db');		
		}
		
		/*
		 * Get Remote Table Count From External Server
		 */
		 public function getVerificationTableCount(){
		 	$table_type = "BASE TABLE";
			$table_name1 = "^KF002";
			$table_name2 = "^KF002.*CORE";
			$table_name3 = "^KF002.*TEST";
			$table_name4 = "^KF002.*TEST1";
		 ...
		 public function updateMetaData(){
			...
			if(pg_num_rows($query_result) > 0){
				while($row = pg_fetch_array($query_result)){
					$count = $row["mcount"];
					$this->insertMetaRecord('verification', $count);
				}
			}
		 }
		 
		 #Get All Installation Tables
		 public function getVerificationTables(){
		 	$table_type = "BASE TABLE";
			$table_name1 = "^KF002";
			$table_name2 = "^KF002.*CORE";
			$table_name3 = "^KF002.*TEST";
			...
			$this->exec('DELETE FROM _VERIFICATION_TABLE');
			$query_result = $obj->runQuery($sql,'odk_prod');
			if(pg_num_rows($query_result) > 0){
				while($row = pg_fetch_array($query_result)){
					$tablename = $row["table_name"];
					$this->insertVerificationRecord($tablename);
				}
				return TRUE;
			}	
			else{
		 ...
		 public function getTableCountStatus(){
		 	$remotecount = $this->getVerificationTableCount();
		 ...
		 #Insert Installation
		 public function insertVerificationRecord($tablename){
		 	try{
				$this->exec("INSERT INTO _VERIFICATION_TABLE (TABLE_NAME) VALUES('$tablename')");
			}
			catch(Exception $e){
		 ...
		 public function getVerificationRecords(){
		 	$tables;
		 	try{
				$result = $this->query("SELECT TABLE_NAME FROM _VERIFICATION_TABLE ORDER BY TABLE_NAME");
		 ...
		 public function createTables(){
		 	try{
				...
				$this->exec('CREATE TABLE _VERIFICATION_TABLE(ID INTEGER PRIMARY KEY AUTOINCREMENT, TABLE_NAME TEXT NOT NULL)');
			}
			catch(Exception $e){
				echo $this->lastErrorMsg();
			}
