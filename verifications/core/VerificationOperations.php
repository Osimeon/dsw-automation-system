<?php
	include 'VerificationMetaData.php';
	include 'VerificationSurveyFields.php';
	...
	class VerificationOperations {
		...
		public function generateSQL(){
			$obj = new VerificationMetaData();
			$obj -> main();	
			$fields = new VerificationSurveyFields();
			$verifications = $obj -> getVerificationRecords();	
			$ext = new Funcs();
			$dbname = 'odk_prod';
			
			foreach($verifications as $verification){				 
				$currtable = $verification['tablename'];
		...
		public function deleteRecord($result){	
			...
			while ($row = pg_fetch_array($result, NULL, PGSQL_ASSOC)){
				...
				if($this -> checkForRecord($_uri) == true){
					$sql = "DELETE FROM verification_survey.verification_survey WHERE _uri = '$_uri'";
					$query_result = $ext -> runQuery($sql,'dsw_db_v2');
				}
			}
		}
	...
	$obj = new VerificationOperations();
?>