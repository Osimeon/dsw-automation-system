<?php
	...
	class ChlDeliveryMetaData extends SQLite3{
		function __construct(){
			$this->open('delivery_database.db');		
		}
		
		/*
		 * Get Remote Table Count From External Server
		 */
		 public function getChlDeliveryTableCount(){
		 	$table_type = "BASE TABLE";
			$table_name1 = "^KF006";
			$table_name2 = "^KF006.*CORE";
			$table_name3 = "^KF006.*TEST";
			$table_name4 = "^KF006.*TEST1";
		 ...
		 public function updateMetaData(){
			...
			if(pg_num_rows($query_result) > 0){
				while($row = pg_fetch_array($query_result)){
					$count = $row["mcount"];
					$this->insertMetaRecord('delivery', $count);
				}
			}
		 }
		 
		 #Get All Delivery Tables
		 public function getChlDeliveryTables(){
		 	$table_type = "BASE TABLE";
			$table_name1 = "^KF006";
			$table_name2 = "^KF006.*CORE";
			$table_name3 = "^KF006.*TEST";
			...
			$this->exec('DELETE FROM _DELIVERY_TABLE');
			$query_result = $obj->runQuery($sql,'odk_prod');
			if(pg_num_rows($query_result) > 0){
				while($row = pg_fetch_array($query_result)){
					$tablename = $row["table_name"];
					$this->insertChlDeliveryRecord($tablename);
				}
				return TRUE;
			}	
			else{
		 ...
		 public function getTableCountStatus(){
		 	$remotecount = $this->getChlDeliveryTableCount();
		 ...
		 #Insert Delivery
		 public function insertChlDeliveryRecord($tablename){
		 	try{
				$this->exec("INSERT INTO _DELIVERY_TABLE (TABLE_NAME) VALUES('$tablename')");
			}
			catch(Exception $e){
		 ...
		 public function getChlDeliveryRecords(){
		 	$tables;
		 	try{
				$result = $this->query("SELECT TABLE_NAME FROM _DELIVERY_TABLE ORDER BY TABLE_NAME");
		 ...
		 #Get Local MetaData Count Of Delivery Tables
		 ...
		 public function createTables(){
		 	try{
				...
				$this->exec('CREATE TABLE _DELIVERY_TABLE(ID INTEGER PRIMARY KEY AUTOINCREMENT, TABLE_NAME TEXT NOT NULL)');
			}
			catch(Exception $e){
				echo $this->lastErrorMsg();
			}
