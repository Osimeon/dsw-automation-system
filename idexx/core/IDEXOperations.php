<?php
	include 'IDEXMetaData.php';
	include 'IDEXSurveyFields.php';
	
	class IDEXXOperations {
		...
		public function generateSQL(){
			$obj = new CEMMetaData();
			$obj -> main();	
			$fields = new IDEXXSurveyFields();
			$idexxs = $obj -> getIDEXXRecords();	
			$ext = new Funcs();
			$dbname = 'odk_prod';
			
			foreach($idexxs as $idexx){				 
		...
		public function generateRows($result, $fields){			
			while ($row = pg_fetch_array($result, NULL, PGSQL_ASSOC)){
				$sql = "INSERT INTO idexx.idexx_survey($fields) VALUES(";
				$count = count($row);
				$y = 0;
				while ($y < $count){
					$c_row = current($row);					
