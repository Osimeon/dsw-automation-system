<?php
	include 'CEMVillageMetaData.php';
	include 'CEMVillageSurveyFields.php';
	
	class CEMVillageOperations {
		public function __construct(){
			$this -> generateSQL();
		}
		
		public function generateSQL(){
			$obj = new CEMVillageMetaData();
			$obj -> main();	
			$fields = new CEMVillageSurveyFields();
		...
		public function checkForRecord($_uri){
			$sql = "SELECT count(_uri) as recs FROM cem_survey.cem_village_contact WHERE _uri = '$_uri'";
			$ext = new Funcs();
		...
		public function deleteRecord($result){	
			...
			while ($row = pg_fetch_array($result, NULL, PGSQL_ASSOC)){
				...
				if($this -> checkForRecord($_uri) == true){
					$sql = "DELETE FROM cem_survey.cem_village_contact WHERE _uri = '$_uri'";
					$query_result = $ext -> runQuery($sql,'dsw_db_v2');
				}
			}
		}
		...
		public function generateRows($result, $fields){			
			while ($row = pg_fetch_array($result, NULL, PGSQL_ASSOC)){
				...
				if($this -> checkForRecord($_uri) != true){	
					...
					$sql = "INSERT INTO cem_survey.cem_village_contact($fields) VALUES(";
					$count = count($row);
					$y = 0;
					while ($y < $count){
						$c_row = current($row);					
