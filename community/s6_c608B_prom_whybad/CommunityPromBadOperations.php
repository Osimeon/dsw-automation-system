<?php
	include 'CommunityPromBadMetaData.php';
	include 'CommunityPromBadSurveyFields.php';
	
	class CommunityPromBadOperations {
		public function __construct(){
			$this->generateSQL();
		}
		
		public function generateSQL(){
			$obj = new CommunityPromBadMetaData();
			$obj -> main();	
			$fields = new CommunityPromBadSurveyFields();
		...
		public function checkForRecord($_uri){
			$sql = "SELECT count(_uri) as recs FROM community_survey.s6_c608B_prom_whybad WHERE _uri = '$_uri'";
			$ext = new Funcs();
		...
		public function deleteRecord($result){	
			...
			while ($row = pg_fetch_array($result, NULL, PGSQL_ASSOC)){
				...
				if($this -> checkForRecord($_uri) == true){
					$sql = "DELETE FROM community_survey.s6_c608B_prom_whybad WHERE _uri = '$_uri'";
					$query_result = $ext -> runQuery($sql,'dsw_db_v2');
				}
			}
		}
		...
		public function generateRows($result, $fields){			
			while ($row = pg_fetch_array($result, NULL, PGSQL_ASSOC)){
				...
				if($this -> checkForRecord($_uri) != true){	
					...
					$sql = "INSERT INTO community_survey.s6_c608B_prom_whybad($fields) VALUES(";
					$count = count($row);
					$y = 0;
					while ($y < $count){
						$c_row = current($row);					
        ...
        $obj = new CommunityPromBadOperations();
?>