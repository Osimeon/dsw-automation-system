<?php
	include 'CommunityPanelMetaData.php';
	include 'CommunityPanelSurveyFields.php';
	
	class CommunityPanelOperations {
		public function __construct(){
			$this->generateSQL();
		}
		
		public function generateSQL(){
			$obj = new CommunityPanelMetaData();
                    	$obj -> main();	
			$fields = new CommunityPanelSurveyFields();
			...
			$sql = "SELECT count(_uri) as recs FROM community_survey.c114c_panel_survey_hhold_round WHERE _uri = '$_uri'";
			$ext = new Funcs();
		...
		public function deleteRecord($result){	
			...
			while ($row = pg_fetch_array($result, NULL, PGSQL_ASSOC)){
				...
				if($this -> checkForRecord($_uri) == true){
					$sql = "DELETE FROM community_survey.c114c_panel_survey_hhold_round WHERE _uri = '$_uri'";
					$query_result = $ext -> runQuery($sql,'dsw_db_v2');
				}
			}
		}
		...
		public function generateRows($result, $fields){			
			while ($row = pg_fetch_array($result, NULL, PGSQL_ASSOC)){
				...
				if($this -> checkForRecord($_uri) != true){	
					...
					$sql = "INSERT INTO community_survey.c114c_panel_survey_hhold_round($fields) VALUES(";
					$count = count($row);
					$y = 0;
					while ($y < $count){
						$c_row = current($row);					
        ...
        $obj = new CommunityPanelOperations();
?>