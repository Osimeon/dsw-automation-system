<?php
	include 'CommunityTalkMetaData.php';
	include 'CommunityTalkSurveyFields.php';
	
	class CommunityTalkOperations {
		public function __construct(){
			$this->generateSQL();
		}
		
		public function generateSQL(){
			$obj = new CommunityTalkMetaData();
			$obj -> main();	
			$fields = new CommunityTalkSurveyFields();
		...
		public function checkForRecord($_uri){
			$sql = "SELECT count(_uri) as recs FROM community_survey.s6_c603_talk_abt_disp_who WHERE _uri = '$_uri'";
			$ext = new Funcs();
		...
		public function deleteRecord($result){	
			...
			while ($row = pg_fetch_array($result, NULL, PGSQL_ASSOC)){
				...
				if($this -> checkForRecord($_uri) == true){
					$sql = "DELETE FROM community_survey.s6_c603_talk_abt_disp_who WHERE _uri = '$_uri'";
					$query_result = $ext -> runQuery($sql,'dsw_db_v2');
				}
			}
		}
		...
		public function generateRows($result, $fields){			
			while ($row = pg_fetch_array($result, NULL, PGSQL_ASSOC)){
				...
				if($this -> checkForRecord($_uri) != true){	
					...
					$sql = "INSERT INTO community_survey.s6_c603_talk_abt_disp_who($fields) VALUES(";
					$count = count($row);
					$y = 0;
					while ($y < $count){
						$c_row = current($row);					
        ...
        $obj = new CommunityTalkOperations();
?>